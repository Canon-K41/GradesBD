...
def update_id_map(table_name):
    ...
    # もしデータがない場合は追加
    for item in df[table_name].unique():
...
def add_id_to_df(table_name):
    ...
    df[column_name + '_id'] = df[column_name].map(id_map)
...
def main():
    ...
    for _, row in df.iterrows():
        ...
        values = (int(row['fields_id']), row['subject_name'], int(row['credit']), int(row['year']), row['term'], int(row['instructors_id']))
    ...
    for _, row in df.iterrows():
        ...
        """
        values = (row['subject_name'], int(row['year']), row['term'], int(row['fields_id']), int(row['evaluation']))
        cursor.execute(query, values)
